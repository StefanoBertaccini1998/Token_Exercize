...
(9) 0xB020D731b501B16BAb58e24F1e4bb134F61582b0 (1000 ETH)*/
  ...
  it("deployed", async () => {
    ...
    expect(this.blackList.address).to.not.equal(ZERO_ADDRES);
  ...
  it("insert user in blackList", async () => {
    ...
    await this.token.insertInBlackList(fourthAccount, { from: deployer });
    console.log(
      await this.blackList.getBlackListStatus({ from: this.blackList.address })
    );
  });
